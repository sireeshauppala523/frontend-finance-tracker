...
- production: deployed Azure backend URL in `.env.production`

The frontend still falls back to localhost in `src/services/api.ts` if `VITE_API_URL` is not provided.
...
The frontend container serves the built SPA through Nginx on port `8080` and uses `nginx.conf` for SPA routing.
